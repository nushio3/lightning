...
import Text.LaTeX.Author.State (AuthorState)
...
-- | refer to a label.  
ref :: (MonadState AuthorState m, LaTeXC (m a)) => Label -> m a
ref lab = do
  ret <- getReference lab
  LTX.ref $ LTX.rendertex ret

-- | insert a label.
label :: (MonadState AuthorState m, LaTeXC (m a)) => Label -> m a
label lab = do
  ret <- getReference lab
  LTX.label $ LTX.rendertex ret
  
getReference :: (MonadState AuthorState m) => Label -> m Text
getReference lab = do
  map0 <- use labelMap
